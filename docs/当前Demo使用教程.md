...
这里的“确认 / 停机处理 / 播放语音”按钮不直接触发 Node-RED flow。它们属于业务模拟端动作，会调用 demo-server 的业务 API，更新告警状态或创建设备指令。Node-RED 在这个 demo 中主要参与设备上报后的解析、规则判断、告警生成和自动联动指令。

1. 先按 `10.1.3` 再触发一次高温告警。
2. 进入“业务模拟端 / 业务告警处理”，或直接打开 `http://127.0.0.1:5173/#/business/alarms`。
3. 找到未关闭的高温告警，点击“确认”。
   - 观察位置：“业务告警处理”页面本行状态会变化。
4. 选择一种业务动作：
   - 点击“停机处理”：平台生成 `stopMachine -> CNC-001` 指令。
   - 点击“播放语音”：平台生成 `playVoice -> VOICE-SPEAKER-001` 指令。
5. 进入“设备指令与回执”，确认新增了业务指令：
   - `stopMachine -> CNC-001`，或
   - `playVoice -> VOICE-SPEAKER-001`。
   - `command_source` 为 `business`。
6. 进入“设备模拟端 / 设备模拟器”，点击“轮询待执行指令”。
7. 对 `CNC-001` 或 `VOICE-SPEAKER-001` 的待执行指令点击“成功”。如需演示异常，也可以点击“失败”或“超时”。
8. 再次进入“设备指令与回执”，确认业务指令状态变为 `success`、`failed` 或 `timeout`。
9. 回到“平台仪表盘”，确认 `指令成功` 或 `指令失败` 数量变化。
...
| Tab | 作用 | 触发来源 | 写入/发布内容 | 主要影响页面 | 停止或禁用后的现象 |
| --- | --- | --- | --- | --- | --- |
| `09-MQTT接入扩展` | 订阅 EMQX MQTT，上报写入 Kafka raw | 设备模拟端 MQTT 上报或真实 MQTT 设备消息 | `iot.telemetry.raw` | 数据事件与消息日志、告警与规则命中、设备指令与回执 | MQTT 上报不再进入后续规则链路 |
| `02-Kafka原始遥测消费` | 消费 `iot.telemetry.raw`，进入标准化 | HTTP / MQTT 上报进入 Kafka 后 | 转给标准化 link | 数据事件与消息日志 | raw 消息不再被 Node-RED 消费处理 |
| `03-数据解析与标准化` | 原始数据解析、校验、写 normalized | raw 消息进入标准化链路 | normalized 遥测、无效数据事件、设备状态更新 | 数据事件与消息日志、设备实例与状态 | 页面不再新增 normalized 数据，设备最新状态不再随上报更新 |
| `04-规则-阈值与无效数据` | 高温阈值规则，生成告警和规则日志 | 标准化遥测消息 | `HIGH_TEMPERATURE` 告警、规则命中日志 | 告警与规则命中、规则与 Node-RED、业务告警处理、平台仪表盘 | 高温上报仍可能入库，但不再生成高温告警和规则命中 |
| `05-规则-连续异常窗口` | 连续 3 次高温触发连续异常告警 | 连续高温遥测 | 连续异常告警、规则命中日志 | 告警与规则命中、业务告警处理、平台仪表盘 | 不再新增连续异常告警 |
| `06-规则-场景联动` | 高温且在线时自动生成 `turnOnAlarm` | 高温规则命中后 | `turnOnAlarm -> ALARM-LIGHT-001` 指令 | 设备指令与回执、设备模拟端待执行指令、平台仪表盘 | 高温告警仍可生成，但不再自动下发声光报警器指令 |
| `07-规则-定时离线巡检` | 每 30 秒巡检离线设备 | `每 30 秒触发` inject 节点 | `DEVICE_OFFLINE` 告警、离线巡检日志 | 告警与规则命中、业务告警处理、规则与 Node-RED、平台仪表盘 | 不再持续新增离线告警；历史离线告警不会自动删除 |
| `10-错误捕获与执行日志` | 统一错误捕获并写规则错误日志 | 其他 flow 运行异常 | 错误执行日志 | 规则与 Node-RED | 异常不再被统一记录到规则执行日志 |

注意：业务模拟端的“确认 / 停机处理 / 播放语音”不直接触发 Node-RED flow，它走 demo-server 业务 API。Node-RED 主要负责设备消息进入平台后的解析、规则、告警和自动联动。
