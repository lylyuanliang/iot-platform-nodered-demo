...
11. 进入“设备指令与回执”，查看业务动作生成的 `stopMachine` 或 `playVoice` 指令。
12. 再次在“设备模拟端 / 设备模拟器”轮询并回执业务指令。
13. 回到“设备指令与回执”和“平台仪表盘”，确认指令状态和成功/失败数量变化。
...
| Tab | 作用 | 触发来源 | 主要影响页面 |
| --- | --- | --- | --- |
| `09-MQTT接入扩展` | MQTT 消息写入 Kafka raw | 设备模拟端 MQTT 上报、EMQX 消息 | 数据事件与消息日志、告警与规则命中、设备指令与回执 |
| `02-Kafka原始遥测消费` | 消费 `iot.telemetry.raw` | HTTP / MQTT 上报写入 Kafka 后 | 数据事件与消息日志 |
| `03-数据解析与标准化` | 原始数据解析、校验、写 normalized | raw 消息进入标准化链路 | 数据事件与消息日志、设备实例与状态 |
| `04-规则-阈值与无效数据` | 高温阈值规则、无效数据处理 | 标准化遥测消息 | 告警与规则命中、规则与 Node-RED、业务告警处理、平台仪表盘 |
| `05-规则-连续异常窗口` | 连续 3 次高温触发严重告警 | 连续高温遥测 | 告警与规则命中、业务告警处理、平台仪表盘 |
| `06-规则-场景联动` | 高温自动生成 `turnOnAlarm` | 高温规则命中后 | 设备指令与回执、设备模拟端待执行指令、平台仪表盘 |
| `07-规则-定时离线巡检` | 定时检查离线设备 | `每 30 秒触发` inject 节点 | 告警与规则命中、业务告警处理、规则与 Node-RED、平台仪表盘 |
| `10-错误捕获与执行日志` | 捕获流程异常并写日志 | 其他 flow 运行异常 | 规则与 Node-RED |

如果禁用某个定时或接入 flow，已经生成的历史数据不会自动删除；变化通常表现为相关页面不再新增对应日志、告警、指令或统计数量。
